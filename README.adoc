...
image::00/00-location.png[Me next to Santa]
...
image::01/00-location.png[Me next to the doves]
...
image::02/00-location.png[Me next to the redacted document]
...
. Open the PDF
+
image::02/01-document.png[Picture of redacted document]
+
. Select all text in the document
+
NOTE: Using `[Ctrl]` + `a`
+
image::02/02-selected.png[Picture of highlighted document]
+
. Copy the text
+
NOTE: Using `[Ctrl]` + `c`
...
+
.Example 4624 event
image::03/00-4624.png[Example 4624 event]
+
...
image::06/00-location.png[Challenge screen]
...
+
image::06/01-hostname.png[Picture showing hostname of sweetums]
+
...
+
image::06/02-hint.png[Hint that gives you a starting search]
+
...
----
+
image::06/03-answer.png[Answer for q02]
+
...
----
+
image::06/04-search.png[Search results]
+
...
----
+
image::06/05-table.png[Search results]
+
...
+
image::06/06-payload.png[Empire payload decoded]
+
...
----
+
image::06/07-no-results.png[No bad results]
+
** Noticed this message in chat
+
image::06/08-chat.png[Chat message]
+
...
----
+
image::06/09-found-bad.png[Table with bad doc]
+
...
** Read the chat
+
image::06/10-chat.png[Chat message]
+
...
----
+
image::06/11-results.png[Search results]
+
...
+
image::06/12-artifacts.png[Email artifacts]
+
...
image::06/13-end.png[Congrats screen]
...
image::07/00-location.png[Room in the dorm]
...
+
image::07/01-tools.png[Network tool]
+
. Enter the room
. Locate the NPC's asset load network event
+
image::07/02-get.png[Krampus asset network event]
+
. Save the asset
+
----
...
----
+
image::07/03-krampus.png[Krampus asset, height=100, width=100]
+
. Cleaned the picture up with GIMP
+
image::07/04-key.png[Plain key]
+
. Decoded the key by looking at the heights relative to each other
+
NOTE: *Key Bitting*: `122520`
+
image::07/07-make-key.png[Crerating a key]
+
image::07/05-122520.png[My key]

image::07/06-win.png[Win screen]
...
image::08/00-location.png[Me next to Krampus]
...
+
image::08/01-site.png[Screenshot of the website]
+
...
----
+
image::08/02-have-pics.png[Showing I have images downloaded]
+
** Manually categorize all of these images :(
+
.Folders for each type
image::08/03-folders.png[Folders of picture types]
+
.All clean and uniform
image::08/04-neat.png[Showing each folder only has images of that type]
. Build a model
** I wanted to try Google's https://teachablemachine.withgoogle.com/[Teachable Machine] for this
+
image::08/05-tm.png[New project screen]
+
** I went with the `Image Project`
** I created a class for each of the image types
+
image::08/06-classes.png[Picture of all classes]
+
...
+
image::08/07-full.png[]
+
** Clicked `Train Model` button
+
image::08/08-training.png[]
+
** Exported my model
+
image::08/09-export.png[]
+
. Solve the CAPTEHA
...
image::08/10-email.png[Winner email]
...
image::10/00-decrypted.png[First page of decrypted document]
...
+
.Adding style
image::11/07-style.png[]
+
.Post change
image::11/08-post.png[]
+
. Lock 07
+
----
...
+
.Force the state
image::11/11-active.png[]
+
.Revealed
image::11/12-done.png[]
. Lock 10
+
----
...
image::11/13-hide.png[]
+
.Inners of the lock
image::11/14-uncovered.png[]
+
...
+
.Copy the components
image::11/15-find.png[]
+
.Ctrl + v it into the lock container
image::11/16-fixed.png[]
...
+
image::12/01-start.png[]
+
** Open *Management* tab
+
image::12/02-settings.png[]
+
** Select *Index Patterns*
+
image::12/03-index-patterns.png[]
+
** Select *Create index pattern*
+
image::12/04-create-pattern.png[]
+
** Add pattern for `zeek-http*`
+
image::12/05-zeek.png[]
+
*** *Time Filter field name* to `@timestamp`
. Look at the data
+
image::12/06-discover.png[]
+
** Where is my data?
+
image::12/07-no-results.png[]
+
** Set the time window for Kibana to look at
+
image::12/08-better.png[]
+
...
+
image::12/13-inspect.png[]
+
image::12/14-data.png[]
+
